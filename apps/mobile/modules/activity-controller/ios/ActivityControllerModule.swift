...
public class ActivityControllerModule: Module {
    @objc func appWillTerminate() {
        if #available(iOS 16.2, *),
            let activity = (getCurrentActivity() as? DefinedActivityWrapper)?.getActivity()
        {
    ...
    public func definition() -> ModuleDefinition {
        ...
        OnCreate {
                ...
                NotificationCenter.default.addObserver(
                    self, selector: #selector(ActivityControllerModule.appWillTerminate),
        ...
        AsyncFunction("startLiveActivity") {
            ...
            do {
                ...
                let activityState = WidgetAttributes.WidgetState(
                    status: args.status, mode: args.mode, totalTime: args.totalTime,
        ...
        AsyncFunction("updateLiveActivity") {
            ...
            guard let activity = (getCurrentActivity() as? DefinedActivityWrapper)?.getActivity()
            else {
            ...
            Task {
                await activity.update(
                    ActivityContent<WidgetAttributes.WidgetState>(
